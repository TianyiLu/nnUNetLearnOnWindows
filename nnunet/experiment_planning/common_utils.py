...
def split_4d_nifti(filename, output_folder):
    ...
    if dim == 3:
        filePath = join(output_folder, file_base[:-7] + "_0000.nii.gz").replace("\\","/")
        ...
        for i, t in enumerate(range(img_npy.shape[0])):
            ...
            path = join(output_folder, file_base[:-7] + "_%04.0d.nii.gz" % i).replace("\\","/")
            sitk.WriteImage(img_itk_new, path)
