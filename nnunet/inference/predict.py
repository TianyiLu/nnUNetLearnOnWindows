...
def predict_save_to_queue(preprocess_fn, list_of_lists, output_files, segs_from_prev_stage, classes):
    #errors_in = []
    result = []
    for i, l in enumerate(list_of_lists):
        try:
            ...
            if np.prod(d.shape) > (2e9 / 4 * 0.9):  # *0.9 just to be save, 4 because float32 is 4 bytes
                ...
                d = output_file[:-7] + ".npy"
            result.append((output_file, (d, dct)))
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=1, segs_from_prev_stage=None):
    ...
    result = predict_save_to_queue(trainer.preprocess_patient,
                                    list_of_lists,
                                    output_files,
