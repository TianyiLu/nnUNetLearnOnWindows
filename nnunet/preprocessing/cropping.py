...
def get_patient_identifiers_from_cropped_files(folder):
    result = [i.split("/")[-1][:-4] for i in subfiles(folder, join=True, suffix=".npz")]
    return result
...
class ImageCropper(object):
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        output_folder_gt = join(self.output_folder, "gt_segmentations")
        output_folder_gt = output_folder_gt.replace("\\", "/")
